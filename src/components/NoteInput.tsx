        ...
        <div className="p-4">
          ...
          {isChecklist ? (
            <div className="space-y-1">
              {checklistItems.filter((item) => !item.isChecked).map((item) => (
              ...
              {checklistItems.filter((item) => item.isChecked).length > 0 && (
                <>
                  <div className="border-t border-gray-200 dark:border-gray-600 my-2" />
                  <button
                    onClick={() => setCompletedCollapsed(!completedCollapsed)}
                    className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 dark:hover:text-gray-400 py-1"
                  >
                    {completedCollapsed ? (
                    ...
                    {checklistItems.filter((item) => item.isChecked).length} completed{' '}
                    {checklistItems.filter((item) => item.isChecked).length === 1 ? 'item' : 'items'}
                  </button>
                  {!completedCollapsed &&
                    checklistItems.filter((item) => item.isChecked).map((item) => (
                      <div key={item.id} className="flex items-center gap-2 group">
                        ...
                        <input
                          ...
                          onChange={(e) => updateChecklistItem(item.id, e.target.value)}
                          placeholder="List item"
                          className="flex-1 bg-transparent text-sm outline-none line-through text-gray-400 dark:text-gray-500"
                        />
                        <button
                          onClick={() => removeChecklistItem(item.id)}
                          className="p-1 opacity-0 group-hover:opacity-100 hover:bg-gray-200 dark:hover:bg-gray-600 rounded"
